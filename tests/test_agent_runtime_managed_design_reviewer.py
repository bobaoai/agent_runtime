...
REPO_ROOT = Path(os.environ.get("AGENT_RUNTIME_REVIEWER_SOURCE_ROOT", Path(__file__).resolve().parents[1]))
pytestmark = pytest.mark.skipif(
    not (REPO_ROOT / ".claude/skills/the-design-authoring/runtime_modules/design_contract_reviewer").is_dir(),
    reason="host integration: set AGENT_RUNTIME_REVIEWER_SOURCE_ROOT to the installed reviewer package",
)
...
def _legacy_review_output() -> dict[str, object]:
    return {
        "verdict": "passed",
        "design_judgment": {
        ...
        "check_results": _valid_review_output()["check_results"],
        "findings": [],
        "safe_next_step": "Use the already registered legacy result.",
    }


def _common_output_schema(schema_ref: str) -> str:
    ...
    return json.dumps({"$schema": "https://json-schema.org/draft/2020-12/schema", "$id": schema_ref,
        ...
        "$defs": {
            ...
            "finding": {"type": "object", "additionalProperties": False, "properties": {"finding_id": string, "severity": {"type": "string", "enum": ["block", "fix", "note"]}, "evidence": {"$ref": "#/$defs/evidence"}, "requirement": string, "impact": string, "accountable_owner_ref": string, "required_change": string}, "required": ["finding_id", "severity", "evidence", "requirement", "impact", "accountable_owner_ref", "required_change"]},
            "check_result": {"type": "object", "additionalProperties": False, "properties": {"check_id": {**string, "enum": list(_required_check_ids())}, "disposition": {"type": "string", "enum": ["passed", "finding", "not_applicable", "not_run"]}, "assessment": string, "finding_ids": {"type": "array", "uniqueItems": True, "items": string}}, "required": ["check_id", "disposition", "assessment", "finding_ids"]}},
        "properties": {"verdict": {"type": "string", "enum": ["passed", "non_pass", "blocked"]}, "check_results": {"type": "array", "items": {"$ref": "#/$defs/check_result"}}, "findings": {"type": "array", "items": {"$ref": "#/$defs/finding"}}, "safe_next_step": string},
...
def _registered_legacy_reviewer():
    ...
    profile = compile_execution_profile_release(ExecutionProfileReleaseSpec(
        execution_profile_id="profile_legacy_design_reviewer_v1",
        executor_adapter_id="claude_agent_sdk_inline_executor", executor_adapter_revision="v2",
        transport_kind="claude_agent_sdk", provider_id="anthropic", model_id="legacy-static",
    ...
    variant_candidate = ExecutionVariantPolicyReleaseCandidate(
        ...
        bindings=(ExecutionVariantProfileBindingCandidate(position_id=MODULE_ID,
...
class _StaticDesignReviewerAdapter:
    ...
    def execute(self, request: AuthorizedAgentExecutionRequest, host) -> AgentExecutionResult:
        ...
        content = json.dumps(self.output, sort_keys=True).encode("utf-8")
