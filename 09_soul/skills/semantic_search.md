...
### 2.1 核心命令
```bash
python 09_soul/tools/semantic_search/main.py \
...
1.  **准备文件列表**：根据需求筛选知识库区域（参考 `09_soul/core/WORKSPACE_REFERENCE.md`，并结合本 repo 实际结构）。
    ```bash
    mkdir -p tmp
    # 示例：搜索当前已导入的调研和记忆目录
    find 09_soul/contexts/survey_sessions 09_soul/contexts/memory -name "*.md" > tmp/search_files.txt
    ```
2.  **执行语义搜索**：
    ```bash
    source .venv/bin/activate
    export OPENAI_API_KEY=$(grep OPENAI_API_KEY .env | cut -d '=' -f2)
    python 09_soul/tools/semantic_search/main.py --file-list tmp/search_files.txt --query "..." --top-k 10 --cache-dir .knowledge_cache
    ```
...
搜索当前本地知识库时，优先考虑以下路径：
- `09_soul/contexts/memory/`：观察与反思。
- `09_soul/contexts/daily_records/`：每日日志占位目录。
- `09_soul/contexts/survey_sessions/`：深度调研结论。
- `09_soul/skills/`：方法论沉淀。
- `04_areas/`：如果真实项目资料在 area 工作面中。

注意：原始 reference repo 中提到的 `contexts/blog/`、`contexts/life_record/`、`contexts/wechat/` 等目录当前未导入到本地环境。这里应以实际存在的目录为准。

以上是常用快捷路径。`--file-list` 可以指向任何文本文件集合，例如：
```bash
# 搜索当前 soul 记忆与调研目录
find 09_soul/contexts -name "*.md" > tmp/search_files.txt

# 搜索某个 area 项目的所有素材
find 04_areas/<your-project> -name "*.md" > tmp/search_files.txt

# 搜索任意临时文档
ls 00_inbox/*.md > tmp/search_files.txt
```
