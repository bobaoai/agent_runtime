...
原因：
- `09_soul/tools/` 下的 CSS 和 HTML 片段已导入
- 但原始发布项目 `adhoc_jobs/yage_share/` 未导入到当前本地环境
- 因此这份 skill 暂保留为方法论和未来恢复模板，不视为当前可直接执行的本地能力

如需恢复：
- 补齐 `yage_share` 项目骨架
- 将文中 `tools/` 和 `adhoc_jobs/` 路径统一本地化到当前 repo
