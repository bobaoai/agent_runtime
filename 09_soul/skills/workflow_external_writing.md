...
这是操作主干。这里的每一条要么是**工件规格**，要么是**可执行、能阻断的 gate**。展开性的原则叙述放在 `bestpractice_external_prose.md`。
...
Main Agent 是编辑、事实负责人和最终验收者。Prose 判定交给看不到 contract 的独立冷读与确定性 CLI。Main Agent 不得凭个人语感点修 Writer 的 prose（错字/数字/路径这类能与 source contract 对照唯一确定的机械修正除外）；需要品味判断的 prose 问题回给管线重跑。
...
### 4.1 全管线共用的 finding 极性与保真边界

所有 Writer、Prose QA、Manager Voice Pass 和终端冷读使用同一极性：**finding 只表示缺陷存在；没有 finding 才表示通过**。不要把“每段是否能回答……”这类正向问题和“是否存在教材声……”这类负向问题混成一个 yes/no verdict。

每轮 prose acceptance 都检查：

1. 是否有术语先于其现象、动作、身份或后果出现，且不满足正式定义例外。正式定义可以先到，但首次出现必须同时说明普通语言角色或分析后果。
2. 是否存在一段，让冷读者无法回答“发生了什么、为什么重要、意味着什么”。
3. 是否存在翻译腔、教科书口吻、机械编号式展开或不自然的概念堆叠。
4. 是否有一处新概念引入速度超过读者的认知负荷。
5. 是否有相邻段落没有形成连续推理，只是互不相干的判断列表。
6. 是否有改文建议可能改变事实、数字、URL、来源归属、因果方向、claim 强度、不确定性、置信边界、场景条件或时间口径。

前五项由 prose 管线重写。第六项只允许 Main Agent 回到 `source_contract.md` 或分析 owner 裁决；Prose QA 和冷读者只能报风险，不得自行改成新的分析结论。
...
- **缺陷 findings**：按 §4.1 的六项逐项报告。finding 代表缺陷存在；不得把正向能力描述记作 finding。
