...
- **Languages**: this root router is English; the active companion skills in `skills/` are Chinese-first. No separate `skills_en/` mirror exists in this repository.
...
## Shared prose acceptance contract

Both routes use one polarity for prose findings: **a finding means a defect is present; no finding means the check passes**. Never mix positive questions such as "can the reader follow this?" with defect questions such as "is a definition premature?" in the same verdict list.

Run these six defect checks on any multi-paragraph reader-facing prose:

1. Does a new term arrive before the reader has seen the phenomenon, action, identity, or consequence it names? A formal definition may come first when precision requires it, but its plain-language role or consequence must appear at first use.
2. Is there a paragraph after which a cold reader cannot answer what happened, why it matters, or what it means for the current judgment?
3. Does the prose contain translation residue, textbook posture, mechanical numbered exposition, or an unnatural pile-up of concepts?
4. Does the rate of new concepts exceed what the reader can keep active without rereading or building an external glossary?
5. Do adjacent paragraphs read as unrelated judgments instead of one continuous line of reasoning?
6. Could a proposed clarity edit change a number, source attribution, causal direction, uncertainty, confidence, scenario condition, time basis, or other governing boundary?

Checks 1-5 route back to prose revision. Check 6 routes back to the analytical or source-owning step; a prose reviewer may identify the risk but must not decide the new meaning.
...
The CLI is a deterministic hygiene floor, not a prose-taste verdict. The external writing workflow uses its exit code as Gate 1, then relies on an independent cold read for judgmental defects. A natural-language "scanned it, looks fine" with no command output is defined as a gate failure.
...
2. Add a pointer to `skills/writing_workflows.md` (this file) in your workspace's skill discovery chain, such as `AGENTS.md`, `CLAUDE.md`, or a skill index like `rules/skills/INDEX.md`. Expose only this root skill; it links to the rest.
