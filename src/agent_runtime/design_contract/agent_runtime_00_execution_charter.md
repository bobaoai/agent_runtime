...
non_goals:
  - product deployment topology and Cell isolation, owned by agent_runtime_02
...
Graph Driver, Executor Adapter, `prompt_bundle`, model profile, tool policy,
evaluator, artifact schema, durable adapter, and worker are separately pinned
release dependencies rather than unnamed parts of a second executable layer.
...
| --- | --- |
| Provider timeout, CLI interrupt, temporary network error | Preserve `domain_state`; record `retryable_failure`; apply execution-profile retry policy |
...
1. **Runtime Release Registry**: Skill Package, Prompt Bundle, Execution
   Profile, Runtime Module, Workflow, admission, and active-release records.
...
Package, Prompt Bundle, Execution Profile, Module, and Workflow Release
