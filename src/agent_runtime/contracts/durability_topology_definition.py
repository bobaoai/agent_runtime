"""Product-topology contracts shared by durable Agent Runtime backends.

The module deliberately carries no domain graph.  It validates the
tenant, cell, entitlement, identity, and ref-only payload boundaries that every
...
class Region(StrEnum):
    """Deployment regions exercised by the first product conformance fixture."""

    US = "us"
    CN = "cn"


class LlmSupply(StrEnum):
    """Whether a cell uses platform-managed or institution-owned model access."""
...
class DeploymentMode(StrEnum):
    """Supported cell resource-sharing modes."""
...
@dataclass(frozen=True)
class CellRuntimeBinding:
    """Server-owned routing and isolation references for one product cell."""
    ...
    def validate(self) -> None:
        """Validate Cell identity and every opaque infrastructure reference."""
        ...
        for label, value in (
            ...
            ("audit_store_ref", self.audit_store_ref),
            ("model_credential_ref", self.model_credential_ref),
...
class PrincipalContext:
    """Authenticated principal metadata used by the tenant router."""
    ...
    def validate(self) -> None:
        ...
        _validate_id("principal_id", self.principal_id)
        _validate_id("tenant_id", self.tenant_id)


class TenantRouter:
    """Resolve tenant and cell from authenticated principal state.

    A caller-provided tenant id is only a consistency assertion.  The server-side
    principal binding remains authoritative.
    """

    def __init__(
        self,
        *,
        principals: Sequence[PrincipalContext],
        cells: Sequence[CellRuntimeBinding],
    ) -> None:
        principal_map: dict[str, PrincipalContext] = {}
        cell_map: dict[str, CellRuntimeBinding] = {}
        for cell in cells:
            cell.validate()
            if cell.tenant_id in cell_map:
                raise ValueError(f"duplicate tenant cell binding: {cell.tenant_id}")
            cell_map[cell.tenant_id] = cell
        for principal in principals:
            principal.validate()
            if principal.principal_id in principal_map:
                raise ValueError(f"duplicate principal: {principal.principal_id}")
            if principal.tenant_id not in cell_map:
                raise ValueError(
                    f"principal has no cell binding: {principal.principal_id}"
                )
            principal_map[principal.principal_id] = principal
        self._principals = MappingProxyType(principal_map)
        self._cells = MappingProxyType(cell_map)
    ...
    def route(
        ...
        *,
        ...
        """Resolve the server-owned Cell and reject a conflicting tenant claim."""
        ...
        if (
            claimed_tenant_id is not None
            and claimed_tenant_id != principal.tenant_id
        ):
            raise PermissionError(
                "caller tenant claim does not match authenticated principal"
            )
        return self._cells[principal.tenant_id]
...
class ExecutionEnvelope:
    ...
    def validate(self, binding: CellRuntimeBinding) -> None:
        """Validate workflow identity against the routed Cell and artifacts."""
    ...
    def to_backend_payload(self, binding: CellRuntimeBinding) -> dict[str, Any]:
...
class BackendDescriptor:
    ...
    def validate(self) -> None:
        ...
        for label, value in (
            ("supports_dedicated", self.supports_dedicated),
            ("supports_pooled", self.supports_pooled),
            ("requires_external_service", self.requires_external_service),
        ):
            if type(value) is not bool:
                raise ValueError(f"backend {label} must be boolean")
        if not self.supports_dedicated:
            raise ValueError("product backend must support dedicated cells")
...
def normalize_backend_descriptor(descriptor: object) -> BackendDescriptor:
    ...
    normalized = BackendDescriptor(
        ...
        supports_dedicated=values["supports_dedicated"],
        supports_pooled=values["supports_pooled"],
...
def assert_dedicated_cell_isolation(cells: Sequence[CellRuntimeBinding]) -> None:
    """Reject dedicated cells that share a data, credential, or backend boundary."""

    if len(cells) < 2:
        raise ValueError("dedicated isolation check requires at least two cells")
    fields = (
        "tenant_id",
        "cell_id",
        "artifact_store_ref",
    ...
    for cell in cells:
        cell.validate()
        if cell.deployment_mode is not DeploymentMode.DEDICATED:
            raise ValueError("dedicated isolation check received pooled cell")
    for field in fields:
        values = [getattr(cell, field) for cell in cells]
        if len(values) != len(set(values)):
            raise ValueError(f"dedicated cells share {field}")
