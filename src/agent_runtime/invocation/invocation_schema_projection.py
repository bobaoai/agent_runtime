...
def transform_json_schema_nodes(
    ...
    JSON Schema uses maps such as ``properties`` and ``$defs`` whose keys are
    user-defined names. Those maps are containers, not schema nodes. Only each
    map value is recursively transformed.
    """
