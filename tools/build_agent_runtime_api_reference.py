"""Generate the selected Runtime API reference without importing Runtime.
...
API_SOURCES = {
    ...
    "src/agent_runtime/execution/execution_local_invocation.py": (
        "prepare_local_workflow", "prepare_local_workflow_module", "run_local_workflow_module",
    ),
    ...
    if isinstance(node, ast.ClassDef):
        ...
        for method in node.body:
            if isinstance(method, (ast.FunctionDef, ast.AsyncFunctionDef)) and (
                ...
                description = (_doc(method, qualified) if method.name != "__init__"
                               or ast.get_docstring(method) else "")
